...
for /l %%i in (%2,1,%1) do mkdir disco-test-client-%%i

echo Populating requests
for /l %%i in (%2,1,%1) do loadtest-populate_test-client-folder.bat disco-test-client-%%i

echo Copying CC-Disco Configs (Creating Exchanges, Queues)
for /l %%i in (%2,1,%1) do copy_cc-disco-configs.bat disco-test-client-%%i