...
grep -E "Warning|Error|warn|fail|crit" C:\ABS\cc-platform\logs\site\disco-service-%DateHourLast%.log   | grep -Ev "CleanupTrackedErrors|is not registered for routing|could not be reached, retrying in" >> %LogFolder%\disco-service_Warnings-Errors.log
grep -E "Warning|Error|warn|fail|crit" C:\ABS\cc-platform\logs\site\siemens-adapter-%DateHourLast%.log | grep -v  "CleanupTrackedErrors" >> %LogFolder%\siemens-adapter_Warnings-Errors.log
